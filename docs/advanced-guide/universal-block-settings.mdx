...
- Shared Blocks
  ...
  Duplicating only creates a reference to the shared Block in the workflow as a new Node. Both Nodes point to the same Block with the same code, so [configuring](/docs/advanced-guide/block-advanced-usage/universal-block-settings#block-configuration) the shared Block will affect both Nodes simultaneously.
...
Refer to [Node.js template](/docs/workflow-engine/nodejs-sdk-api#21-scriptlet-template) and [Python template](/docs/workflow-engine/python-sdk-api#21-scriptlet-template)
