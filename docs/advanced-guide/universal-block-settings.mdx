---
sidebar_position: 1
sidebar_label: Universal Settings
---
...
This article describes the settings and behaviors shared by all Blocks.

## Node Menu

When a Block is used inside a workflow, it appears there as a Node, and the Node menu becomes available.
...
The Node menu provides two execution methods. The main difference is whether the current run reuses cache from a previous successful run. Both methods start from the necessary upstream graph and execute up to the current Node.
...
Some Blocks can be expensive or slow to execute, so rerunning them every time would be wasteful. That is why OOMOL provides a Node-level cache mechanism.
...
Users can also choose "Clear cache and execute", which skips cache reuse for the current run, triggers upstream execution again, and refreshes the output values.
...
Node Settings only affect the current workflow. Each Node's settings are independent and do not affect other Nodes.
...
If you are authoring Blocks in YAML and need the recommended rules for `oomol/secret`, OOMOL auth, Fusion-first decisions, or `sessionDir` fallback, see [Secrets, Auth, and Runtime Paths](/docs/advanced-guide/secrets-auth-and-runtime-paths).
