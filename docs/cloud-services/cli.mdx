...
managed installs, OOMOL accounts, published packages, connector actions, cloud
...
skills, and keeping the active CLI version current through official installer
scripts and managed updates.

## What `oo-cli` is good for

- Install the managed CLI binary from official scripts or a package manager, then keep it current with `oo update`.
...
Package-manager install remains available:
