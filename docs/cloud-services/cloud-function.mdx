...
If you want the fastest way to use a published OOMOL function, start with the
[oo-cli guide](/docs/oo-cli). If you need the concrete CLI namespaces
and commands, use the
[Command Reference](/docs/oo-cli/command-reference). If you need to
call Cloud Function programmatically, use the docs in **API & SDK**. The
current execution endpoints and SDK packages still use the name
`Cloud Task v3`, which is the current integration layer behind Cloud Function.
...
Use [`oo`](https://github.com/oomol-lab/oo-cli) to authenticate, search
packages or connector actions, inspect package metadata, run cloud tasks, move
files, and install bundled skills for AI coding hosts.

**Use Cases:**
- Use OOMOL functions from Codex or Claude Code
- Let individual users run cloud functions without building an app
- Search both package-backed and connector-backed paths before choosing one
...
See the [oo-cli guide](/docs/oo-cli) for the recommended workflow and the
[Command Reference](/docs/oo-cli/command-reference) for current commands such
as `oo search`, `oo packages info`, and `oo cloud-task run`.
...
For the current API shape and SDK naming, see [Cloud Function API Overview](/docs/cloud-services/cloud-task/overview).
...
- **Usage-based billing**: Pay only for hosted runtime after your included monthly usage is used up
- **Transparent pricing**: Cloud Task is billed by runtime, and anything under one minute is billed as one minute
- **Free tier available**: Get started with included monthly Cloud Task usage before you need to recharge or upgrade
...
After deployment, most users should start with the
[oo-cli guide](/docs/oo-cli). Cloud Function currently uses the
`Cloud Task v3` integration layer for direct API access. For the current CLI
command surface, see the
[Command Reference](/docs/oo-cli/command-reference). For a working
request example, see
[HTTP API With curl](/docs/cloud-services/cloud-task/curl-api).
...
- Check the [Documentation Center](/docs/overview)
