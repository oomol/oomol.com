...
Use [`oo-cli`](https://github.com/oomol-lab/oo-cli) to search packages, inspect package metadata, run cloud functions, and fetch results directly from the terminal.
...
See the [CLI guide](/docs/cloud-services/cli) for the recommended workflow.
...
After deployment, most users should start with the [CLI guide](/docs/cloud-services/cli). Cloud Function currently uses the `Cloud Task v3` integration layer for direct API access. For a working request example, see [HTTP API With curl](/docs/cloud-services/cloud-task/curl-api).
