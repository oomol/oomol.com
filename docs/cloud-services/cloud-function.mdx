...
If you need to call Cloud Function programmatically, use the docs in **API & SDK**. The current execution endpoints and SDK packages still use the name `Cloud Task v3`, which is the current integration layer behind Cloud Function.
...
## Three Ways to Deliver the Same Capability

Cloud Function works alongside two other delivery options. They serve different goals, but all start from the same reusable logic.
...
After deployment, OOMOL provides the endpoint details you need for direct integration. For the current API shape and SDK naming, see [Cloud Function API Overview](/docs/cloud-services/cloud-task/overview).

### 3. Expose as MCP for AI
...
Cloud Function currently uses the `Cloud Task v3` integration layer for direct API access. For a working request example, see [HTTP API With curl](/docs/cloud-services/cloud-task/curl-api).
