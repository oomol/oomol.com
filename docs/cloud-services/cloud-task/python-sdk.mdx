...
`oomol-cloud-task-sdk` is the Python client for OOMOL Cloud Task API v3.
...
`webhookUrl` and `metadata` remain in the request type only for backward compatibility. The client ignores them before sending the request.
