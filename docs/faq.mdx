...
1. If you are in mainland China, enable the China mirror source in `Settings -> Preferences -> China Mirror Source`.
2. Ensure the current system time is accurate. Large discrepancies may lead to SSL certificate verification failures.
...
## Docker Desktop for Windows Throws This Error `wsl distro proxy in ovm-oomol-studio distro has exited with an error: exit status 1`
...
1. `configuring docker in ovm-oomol-studio: docker cli config: failed to write file: exit status 0xffffffff`
...
**We recommend converting these script blocks into shared task blocks before publishing. That avoids the dependency-sync problems described above.**
...
## Common Flow Authoring Errors

If a Flow cannot run because of validation or wiring errors, the following problems are the most common:

| Problem | Typical cause | What to check |
| --- | --- | --- |
| Unknown Handle | The Handle name is misspelled or uses different capitalization | Check that the connected Handle names match exactly |
| Required input missing | A required Handle is not connected | Wire the input, or mark it nullable only when it is truly optional |
| Task or subflow reference not found | The YAML reference form is wrong | For local reusable units, use forms such as `task: self::{name}` or `subflow: self::{name}` |
| Slot output does not arrive | The slot contract and slotflow output do not use the same Handle name | Check that the slot output name and slotflow output name are identical |
| No public output from a subflow | The subflow exposes no outward routing | Check whether the subflow has a root-level `outputs_from` |
| Circular dependency | A downstream Node is wired back into an upstream Node | Remove the direct cycle and redesign the data flow |

If your issue is specifically about subflows, slots, or slotflows, see [Subflow Block Advanced Usage](/docs/advanced-guide/advanced-subflow-block).
...
If you start a MySQL service on your computer, you can connect to `127.0.0.1:3306` on the host machine. In OOMOL Studio, connect to `host.docker.internal:3306`.
