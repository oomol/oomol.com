...
That means when you are building Scriptlet Blocks or Task Blocks inside Studio, you can usually use the Fusion SDK directly without manually installing it first.
...
If you are running outside Studio, or if you need an explicitly managed package version in your own app, then install the SDK in the normal way.
