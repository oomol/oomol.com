...
Before we officially start, you can open this example directly from the community catalog by [clicking here](https://hub.oomol.com/package/nasa-apod-quickstarts?open=true).
...
If you want the authoring rules behind these secrets, including `oomol/secret`, OOMOL auth, and when to prefer Fusion, see [Secrets, Auth, and Runtime Paths](/docs/advanced-guide/secrets-auth-and-runtime-paths).

As a simple quickstart, we have completed a full workflow. In subsequent chapters, we will cover more interesting and practical features and uses of OOMOL Studio.
