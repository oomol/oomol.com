...
hide_title: true
description: Start with oo-cli through official install and update paths, build in OOMOL Studio when published tools stop short, and deliver through OOMOL Cloud.
---
...
>
  Start with oo-cli. Install it from the official script or your package
  manager. Search, inspect, and run published tools first. Move into Studio
  when published tools stop short. Deliver through Cloud when it needs to keep
  running.
</div>

These docs follow the same path as the main site. If you only need to use
published tools, start with `oo-cli`. The current CLI path now includes
official installer scripts, managed updates, bundled skills for Codex and
Claude Code, connector execution, Cloud Task operations, files, logs, and
shell completion. If you need to build or extend your own tools, move into
OOMOL Studio. If the validated tool needs hosting, hosted delivery, or managed
access, continue with OOMOL Cloud.
