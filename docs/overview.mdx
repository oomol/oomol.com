...
>
  Start with oo-cli. Build in Studio when ready-made tools stop short. Deliver
...
move into OOMOL Studio. If the validated tool needs hosted runtime, delivery,
...
Move to OOMOL Studio when ready-made tools stop short and you need to build or
...
Use OOMOL Cloud after the implementation is validated and needs hosted runtime
or delivery.
...
- Continue exposing the same capability through APIs, MCP, automation, and `oo-cli`
...
- [Cloud Function](/docs/cloud-services/cloud-function): use this when the validated tool needs hosted delivery and runtime
