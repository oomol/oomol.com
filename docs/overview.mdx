...
graph TB
    subgraph Layer4["Application Layer"]
        Hub["OOMOL Flowhub<br/>(Project & Block Sharing Hub)"] ~~~ Cloud["OOMOL Cloud<br/>(Serverless Cloud Function Deployment)"]
    end

    subgraph Layer3["Platform Layer"]
        Studio["OOMOL Studio<br/>(All-in-One Desktop Software)"] ~~~ Headless["OOMOL Headless<br/>(Deployable Image)"]
    end
