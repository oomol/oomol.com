...
Block functions can be written in either `Node.js` or `Python`. Each runtime has its own entry convention:
1. Python usually uses a `main` function
2. Node.js usually uses the module's default exported function

In both cases, the callable receives two parameters:
1. Input data
2. Runtime helpers provided by the block context

The function should return a dictionary/object. In `Node.js`, that can be a `Promise` that resolves to an object. The returned top-level fields become the block's output data.
...
Currently, `blocks` support `Node.js` and `Python` runtime environments. Each runtime environment has a corresponding [Executor](/docs/concepts/executor) that loads and executes the block code.
...
Blocks running in different language environments exchange data through JSON. Blocks in the same language environment can exchange data either through JSON or by passing variables directly.
