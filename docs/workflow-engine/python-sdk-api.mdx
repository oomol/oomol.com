...
Below is the standard Scriptlet template. Write your execution logic inside the function body. The input and output types can be customized, and third-party libraries are supported.
To pass data to downstream blocks, return a dictionary as you would in a normal function. OOCANA uses the top-level keys as output handles, and the corresponding values as the outputs for those handles.
