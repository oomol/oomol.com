...
然后我们分别使用 `pdf-craft` 包中的 `Analys PDF` 和 `Genrate EPUB` 区块，`books-translator` 包中的 `Translate epub book` 区块，`File` 包中的 `Binary save as file` 区块，串联后得到工作流。
...
1. 需要处理的 pdf 文件路径
2. 最终输出 epub 文件的路径和文件格式

`Translate epub book` 区块默认的输入语言是英文，输出语言是中文。如果有需要请自行调整参数。

> 这里可以清空 `output_dir`，让中间产物都放在内存中，关闭应用即可释放。
