...
date: 2025-07-05
description: "了解如何把 OOMOL 工作流导出成 OCI 镜像、部署成后台服务，并运行一个 PDF 转 EPUB 翻译流程。"
...
在得到镜像文件后，我们可以使用 `docker` 这类容器运行时命令加载镜像文件。在终端中运行
...
接口的具体描述可以参考 `http://localhost:3000/ui` 暴露的 API Schema。
