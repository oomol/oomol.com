...
有些任务区块预期会比普通任务运行更久。在 YAML 中，这通常通过 `long_running` 表达：

```yaml
long_running: true
```

只有在任务确实属于长时任务时才建议这样设置，例如：

- 任务通常会运行 10 分钟以上。
- 运行时长会随着输入规模显著增长。
- 它更像一种后台处理，而不是一次短同步调用。

不要因为某一次执行刚好比较慢，就把任务标记成长时任务。一次性的 API 调用、短时间格式转换、普通文件操作，一般都应保持默认行为。

如果你是直接编辑 YAML，可以记住这条简单规则：

- 默认省略 `long_running`，或保持 `false`。
- 只有在任务明确属于长时或随输入扩张明显变慢时，才使用 `true`。

如果你还想看 secret、OOMOL 认证、Fusion 优先以及运行时路径回退这类相邻规则，可以继续参考 [Secrets、认证与运行路径](/zh-CN/docs/advanced-guide/secrets-auth-and-runtime-paths)。

## 在任务区块中使用 Fusion

很多任务区块的实际作用，就是把一次 Fusion 调用封装成一个稳定、可复用的输入输出接口。

通常下面这些场景都适合这样做：

- 目标能力已经由 OOMOL Fusion 提供
- 你希望用户只填写业务参数，而不是再配置外部模型 key
- 你希望认证直接走 OOMOL token 流程

在 Studio 里，建议直接从运行时 `context` 初始化 SDK：
...
任务区块的配置，除`自定义 UI`外，其余部分都与[通用区块设置](/zh-CN/docs/advanced-guide/universal-block-settings)一致。

由于 OOMOL Studio 提供的[Node.js 预览](/zh-CN/docs/workflow-engine/nodejs-sdk-api#contextpreview-types)和[Python 预览](/zh-CN/docs/workflow-engine/python-sdk-api#contextpreview-类型)覆盖的展示场景有限，因此我们提供了自定义 UI，让区块作者自行决定区块应该如何渲染或预览内容。
