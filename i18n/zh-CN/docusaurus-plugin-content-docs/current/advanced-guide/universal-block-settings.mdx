...
这篇文章主要介绍所有区块共享的一些设置项和行为规则。

## 节点菜单

当一个区块被放进工作流中使用时，它会以“节点”的形式出现，此时对应的节点菜单也会可用。
...
节点菜单中有两种执行方式，核心区别在于本次运行是否复用上一次成功运行的缓存。两种方式都会从必要的上游开始执行，一直到当前节点为止。
...
有些区块可能非常耗时，或者资源消耗很高，用户往往不希望每次都重复执行，因此我们设计了节点级缓存机制。
...
用户也可以选择“清除缓存并执行”。这样本次运行就不会复用缓存，而是重新触发上游节点执行，以获取最新输出。
...
节点配置只会在当前工作流内生效。每个节点的配置彼此独立，不会相互影响。
...
考虑到用户可能在某些场景下使用诸如 API Key 之类的私密字符串，我们提供了密钥功能，以防止敏感信息泄露。

用户可以在首页的[密钥管理页面](/zh-CN/docs/get-started/interface/home/#密钥页面)或者[项目界面内密钥管理页面](/zh-CN/docs/get-started/interface/project#密钥)中添加密钥。

然后就可以在密钥类型的接口下选择使用。

如果你是从 YAML 作者视角来编写区块，并且想看 `oomol/secret`、OOMOL 认证、Fusion 优先、`sessionDir` 回退这些推荐规则，可以继续参考 [Secrets、认证与运行路径](/zh-CN/docs/advanced-guide/secrets-auth-and-runtime-paths)。
