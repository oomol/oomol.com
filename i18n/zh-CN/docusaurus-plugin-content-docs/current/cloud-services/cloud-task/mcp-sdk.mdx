...
`oomol-cloud-mcp-sdk` 是构建在 OOMOL Cloud Task TypeScript SDK 之上的 MCP Server SDK。

它主要承担两类角色：

- 完整 re-export `oomol-cloud-task-sdk`，因此也可以直接当作 Cloud Task TypeScript SDK 使用
- 在此基础上提供 MCP Server 封装，把 Cloud Task 能力暴露成 MCP tools
...
如果希望任务类 tools 默认使用某个 package，可以再额外设置：
...
`packageName` 和 `packageVersion` 可以来自 tool 参数，也可以直接使用 server 的默认配置。
