...
## Docker Desktop for Windows 报错 `wsl distro proxy in ovm-oomol-studio distro has exited with an error: exit status 1`
...
**我们建议在发布前将这些小脚本区块转换为共享任务区块，这样可以避免上面提到的依赖同步问题。**
...
如果一个流因为校验或连线问题无法运行，最常见的情况通常是下面这些：

| 问题 | 常见原因 | 应检查什么 |
| --- | --- | --- |
| 找不到接口 | 接口名拼写错误，或者大小写不一致 | 检查连线两端的接口名是否完全一致 |
| 缺少必填输入 | 必填接口没有连接 | 连接这个输入，或者仅在它确实可选时才标记为可空 |
| 找不到任务或子流引用 | YAML 引用形式写错了 | 对本地复用单元，使用 `task: self::{name}`、`subflow: self::{name}` 这类形式 |
| 插槽输出没有到达 | 插槽契约与 slotflow 输出没有使用相同的接口名 | 检查插槽输出名与 slotflow 输出名是否完全一致 |
| 子流没有公共输出 | 子流没有把内部结果路由到外部 | 检查子流根级别是否定义了 `outputs_from` |
| 循环依赖 | 下游节点又回接到了上游节点 | 去掉这个直接环路，重新设计数据流方向 |

如果你的问题主要和子流、插槽或 slotflow 有关，可以继续参考[子流区块进阶用法](/zh-CN/docs/advanced-guide/advanced-subflow-block)。
...
如果你在计算机上启动了一个 MySQL 服务，那么在本机上可以通过 `127.0.0.1:3306` 连接；在 OOMOL Studio 内，则可以通过 `host.docker.internal:3306` 连接。
