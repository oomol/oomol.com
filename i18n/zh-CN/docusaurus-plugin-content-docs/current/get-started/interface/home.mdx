...
在打开 OOMOL Studio 后，首次映入眼帘的就是项目列表界面。你可以在这里查看到你本地的所有项目，也可以创建、导入、删除项目。

左侧是我们首页的导航栏，点击不同的选项可以切换到不同的页面，页面的作用将在下面介绍。

在左下角是你当前账户的 Token 信息，你可以在这里看到 Token 的使用情况和剩余数量，以及跳转到 Token 的管理页面。

:::info
Token 的作用主要是用于: 代码 AI 补全、AI Agent、AI API 等功能的调用。
:::
...
关于更多的社区介绍可以参考[社区](/zh-CN/docs/community/introduction)。
...
> 为了保持不同系统之间的兼容性问题，我们强制要求“容器目录名” 必须由小写字母 + 数字开头，只能只能包含：小写字母、数字、点、下划线和连字符，且最多 215 个字符。
...
:::info
关于 “OVM 文件夹” 迁移，可以见 [这里](https://oomol.com/zh-CN/updates/OOMOL_Studio_1.1.0#%E6%94%AF%E6%8C%81%E7%A7%BB%E5%8A%A8-ovm-%E8%99%9A%E6%8B%9F%E6%9C%BA%E6%95%B0%E6%8D%AE%E7%9B%AE%E5%BD%95)
:::
