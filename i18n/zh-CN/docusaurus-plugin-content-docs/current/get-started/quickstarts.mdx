...
在正式开始前，你可以先从社区目录里直接打开这个示例，[点击这里](https://hub.oomol.com/package/nasa-apod-quickstarts?open=true)即可在本地进行尝试。
...
这个错误是因为 NASA 的 `DEMO_KEY` 有请求限制。你可以通过申请一个免费的 API Key 来解决这个问题。

在 OOMOL Studio 中，我们可以使用 `密钥` 来存储 API Key。你可以在左侧菜单中找到 `OOMOL 密钥`，然后点击 `添加密钥` 按钮来添加新的密钥，如图所示。
...
然后在 Block 中添加一个 `输入接口`，将类型修改为 `密钥`，再选择你刚刚添加的密钥。接着把代码中的 `DEMO_KEY` 替换为 `${params.input}` 这个接口值。
...
如果你想进一步看这些 secret 背后的编写规则，包括 `oomol/secret`、OOMOL 认证以及何时优先使用 Fusion，可以继续参考 [Secrets、认证与运行路径](/zh-CN/docs/advanced-guide/secrets-auth-and-runtime-paths)。

作为一次简单的快速体验，我们已经完成了一个完整的工作流。后续章节我们将会介绍更多有趣且实用的 OOMOL Studio 的功能和用法。
