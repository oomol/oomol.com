...
sidebar_label: 概述
hide_title: true
description: 先通过官方安装和更新路径用好 oo-cli，用已发布工具跑通；不够时进入 OOMOL Studio；需要持续运行和交付时再交给 OOMOL Cloud。
---
...
>
  先从 oo-cli 开始。先用官方脚本或包管理器装好它，再去搜索、查看和运行已发布
  工具。已发布工具不够时进入 Studio。需要持续运行和交付时再交给 Cloud。
</div>

这份文档遵循主官网现在的产品路径。如果你只是想先把已发布工具跑起来，就从
`oo-cli` 开始。当前这条 CLI 路径已经包含官方安装脚本、受管更新、Codex /
Claude Code 的 bundled skills、connector 执行、Cloud Task、文件、日志和
shell 补全；如果你需要自己编写、扩展和验证工具，就进入 OOMOL Studio；
