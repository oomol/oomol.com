...
大多数情况下，它会在 OOMOL Studio 里被用来制作 block，把 OOMOL 托管的能力封装成可复用区块。除此之外，你也可以在自己的 Node.js 或 Python 服务里直接调用它。
...
这种模式适合不在 Studio 运行、也拿不到运行时 `context` 的场景。
...
- [Fusion SDK TypeScript 文档](./fusion-sdk-typescript)
