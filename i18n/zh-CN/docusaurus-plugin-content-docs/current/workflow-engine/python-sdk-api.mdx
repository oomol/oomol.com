...
开发人员将在 Scriptlet Block 中调用我们的 SDK 与 `OOCANA` 工作流引擎进行交互。

## 2. 快速开始

在 Scriptlet Block 中使用 SDK, 你需要导入 `Context` 对象，然后使用 `Context` 对象中的 API。

### 2.1 `Scriptlet` 模板

模板是一个函数，需要执行的代码写在函数中。函数的输入和输出类型由用户自定义。也支持安装第三方库，直接和正常开发一样引用就可以。
